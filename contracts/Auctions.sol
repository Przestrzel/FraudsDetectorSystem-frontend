...
contract Auctions {
  ...
  struct Offer {
    ...
    uint256 price; // price in gr, no float type
  }
  ...
  function registerAdvertiser(string memory name, string memory city) public {
    ...
    require(
      bytes(advertisers[msg.sender].advertiser_name).length == 0,
      'Advertiser with this account is already registered'
    );
      ...
      if (auctions[auction_id].advertiser_addr == msg.sender) {
        for (uint256 j = 0; j < auctions[auction_id].offersCounter; j++) {
          if (auctions[auction_id].offers[j].offerent_addr == winnerAddr) {
            auctions[auction_id].id_winner = int256(j);
  ...
  function makeOffer(
    ...
    require(
      bytes(offerents[msg.sender].offerent_name).length != 0,
      'You are not registered as an offerent'
    );
    if (strcmp(auctions[auction_id].auction_name, auction_name)) {
  ...
  // Something else
  function getAuctionStatus(uint256 auction_id, string memory auction_name)
    public
    view
  ...
  function getAuctionWinner(uint256 auction_id, string memory auction_name)
    public
    view
    returns (address)
  {
    if (strcmp(auctions[auction_id].auction_name, auction_name)) {
      if (auctions[auction_id].id_winner >= 0) {
        return auctions[auction_id].offers[uint256(auctions[auction_id].id_winner)].offerent_addr;
      } else {
